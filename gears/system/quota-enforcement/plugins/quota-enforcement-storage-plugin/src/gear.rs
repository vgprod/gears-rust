...
/// Storage plugin gear.
///
/// `init` validates the configuration and binds the plugin to the gear's
/// database: the foundation store and the Quota store over the notification
/// outbox handle. The runtime applies the migrations before `init` through
...
/// resolves no storage plugin until the last primitive lands. The outbox
/// handle stays unbound until the notification dispatcher starts the pipeline
/// and binds it; until then every mutation rolls back.
...
impl StoragePluginGear {
    ...
    fn notification_enqueuer(&self) -> Arc<dyn NotificationEnqueuer> {
...
impl Gear for StoragePluginGear {
    async fn init(&self, ctx: &GearCtx) -> anyhow::Result<()> {
        ...
        info!(
            ...
            "storage plugin initialised; client registration waits for the complete contract"
        );
        Ok(())
    }
}
